...
const CreateProfileScreenStepFour = ({
  ...
  const { gender, accountType, firstName, lastName, nickName, city, language } =
    profileState;
  ...
  const getPermissionAndGetPicture = async () => {
    ...
    if (status === "granted") {
      ...
      if (result.cancelled === false) {
        if (
          result.uri.split(".")[result.uri.split(".").length - 1] === "png" ||
          result.uri.split(".")[result.uri.split(".").length - 1] === "jpg"
        ) {
  ...
  const sendPicture = async () => {
    ...
    formData.append(
      "isPersonalAccount",
      accountType === "Personal Account" ? true : false
    );
  ...
  return (
    <View style={styles.profile}>
      <View style={styles.imgContainer}>
        {!selectedPicture ? (
          <DefaultPicture width={120} height={120} />
        ) : (
          <Image style={styles.img} source={{ uri: selectedPicture }} />
        )}
        <TouchableOpacity
          onPress={() => {
      ...
      <View style={styles.bar}>
        <View
          style={[styles.done, { width: selectedPicture ? "100%" : "10%" }]}
        ></View>
      </View>
      {!picture && errorMsg && (
